<div>
    <div class="container mt-3">
        <h1 style="font-size: 25px; font-weight: 500; color: white;" class="text-start mb-5">Permissions</h1>
        @include('livewire.modals.admin-modal')
        @if (session('message'))
            <div class="alert alert-success text-black text-center" id="messagee">{{ session('message') }}</div>
        @endif
        <div class='btn btn-secondary btn-sm float-end mb-2 bg-secondary bg-opacity-25' data-toggle="modal" data-target="#modal-permissions"> Create New Permission</div>
        ...
        <div class="card-body">
            <table class="table shadow text-center text-white border border-light">
                ...
                <tbody  class="text-white bg-secondary bg-opacity-50">
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="" class="btn btn-warning" id="ic" title="Edit" data-toggle="modal" data-target="#update-modal-permissions" wire:click="editPermissions({{ $permission->id }})">Edit</a>
